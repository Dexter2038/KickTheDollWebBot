...
app: FastAPI = FastAPI(title="SPA App", docs_url=None, redoc_url=None)
app.mount('/assets',
          StaticFiles(directory="app/assets", check_dir=False),
          name="assets")
...
async def join_dice_room(request: JoinRequest):
    ...
    if request.room_id not in dice_rooms:
        logger.info(
            f"Пользователь {request.player_id} попытался присоединиться к несуществующей комнате {request.room_id}")
...
async def roll_dice(request: TurnRequest):
    ...
    if request.room_id not in dice_rooms:
        logger.info(
            f"Пользователь {request.player_id} попытался бросить кубики в несуществующей комнате {request.room_id}")
...
def task_mark_guess_games():
    asyncio.run_coroutine_threadsafe(coro=db.mark_guess_games(), loop=asyncio.get_running_loop())


def clear_game_sessions():
    asyncio.run_coroutine_threadsafe(coro=db.clear_game_sessions(), loop=asyncio.get_running_loop())
