...
async def insert_lottery_transaction(
    user_id: int, multiplier: float, amount: float
) -> None:
    """
...
async def clear_game_sessions():
    ...
    async with new_session() as session:
        statement = update(Users).values(
            last_visit_to_bot=datetime.utcnow() - timedelta(hours=5), bonuses_to_bot=3
        )
...
async def get_top_lottery_transactions() -> List[Tuple[str, float, int]]:
    ...
    async with new_session() as session:
        ...
        rows = result.all()
        top_transactions = [
            (await get_username(user_id), multiplier, amount)
            for user_id, multiplier, amount in rows
        ]
    return top_transactions


async def add_user_wallet(telegram_id: int, wallet_address: str) -> bool:
    """
    Adds wallet to user
...
async def remove_user_wallet(telegram_id: int) -> bool:
    """
...
async def take_referral_reward(user_id: int) -> float:
    ...
    async with new_session() as session:
        query = (
            select(db.func.sum(Referrals.bonus))
            .select_from(Referrals)
            .where(Referrals.referrer_id == user_id)
        )
        result = await session.execute(query)
...
async def create_user(telegram_id: int, username: str, wallet_address: str) -> bool:
    """
...
async def get_user(telegram_id: int) -> Users:
    """
...
async def clear_user(telegram_id: int) -> bool:
    """
...
async def add_user_money(telegram_id: int, money: int) -> bool:
    """
...
async def subtract_user_money(telegram_id: int, money: int) -> bool:
    """
...
async def select_transactions():
    ...
    async with new_session() as conn:
        ...
        for transaction in curr.fetchall():
            ...
            new_created_at = (created_at.astimezone(moscow_tz)).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            print(
...
async def create_transaction(
    user_id: str, amount: int | float, transaction_type: int
) -> bool:
    """
...
async def create_bet(
    user_id: int, coin: str, amount: int | float, shift_hours: int, way: int
) -> bool:
    logger.info(
...
async def minus_user_money(telegram_id: int, money: int) -> bool:
    """
...
async def edit_money_balance(telegram_id: int, money_balance: float | int):
    logger.info(
...
async def add_user_money_balance(telegram_id: int, money_balance: float | int):
    logger.info(
...
async def update_referrers_balance(referred_telegram_id: int, bonus_amount: float):
    """
    ...
    async with new_session() as session:
        ...
        query = select(Referrals).where(
            Referrals.referrer_id == referred_telegram_id, Referrals.status == True
        )
        result = await session.execute(query)
...
async def edit_dollar_balance(telegram_id: int, dollar_balance: float | int):
    logger.info(
...
async def get_count_transactions() -> int:
    ...
    """
    async with new_session() as session:
        query = select(db.func.count()).select_from(Transactions)
        result = await session.execute(query)
        count = result.scalar()
        return count
...
async def get_game_params(telegram_id: int) -> Tuple[int, bool, bool, datetime]:
    """
    ...
    async with new_session() as session:
        query = select(
            ...
            (Users.last_visit_to_bot < datetime.utcnow() - timedelta(hours=4)),
...
async def get_sum_lottery_transactions() -> float:
    async with new_session() as session:
        query = select(db.func.sum(LotteryTransactions.amount)).select_from(
            LotteryTransactions
        )
        result = await session.execute(query)
        sum = result.scalar()
...
async def catch_sum():
    async with new_session() as session:
        query = select(db.func.sum(Transactions.amount)).select_from(Transactions)
        result = await session.execute(query)
        sum = result.scalar()
        print(sum)
