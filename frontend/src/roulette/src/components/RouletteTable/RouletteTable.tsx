...
export const RouletteTable: FC<IRouletteTableProps> = ({
    ...
    useEffect(() => {
        ...
        const listener = (event: Event) => {
            ...
            const isActionSupported =
                Object.keys(ACTION_TYPES).includes(action);
    ...
    const handleBetCatcherHover = useCallback(
        (event: MouseEvent<HTMLDivElement>) => {
            const highlightData = (event.currentTarget as HTMLDivElement)
                .dataset.highlight;
            ...
            if (isFromConfig === true) {
                ...
                if (
                    config[firstHighlight as keyof typeof config] === undefined
                ) {
                    console.error(
    ...
    return (
        <RouletteTableContext.Provider value={contextValue}>
            <div
                className={classNames("roulette-table-container", {
                    debug: isDebug,
                })}
                ref={tableRef}
            >
